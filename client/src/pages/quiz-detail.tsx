...
import { Plus, Timer, Award, CheckCircle, XCircle, Edit } from "lucide-react";
...
export default function QuizDetail() {
  ...
  const updateQuestionMutation = useMutation({
    mutationFn: async (questionData: any) => {
      const response = await apiRequest("PUT", `/api/quizzes/${quizId}/questions/${questionData.id}`, questionData);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/quizzes", quizId, "questions"] });
  ...
  const handleCreateQuestion = () => {
    ...
    if (newQuestion.type === "multiple_choice" && newQuestion.options.some(opt => !opt)) {
      toast({
        title: "Validation Error",
  ...
  const handleEditQuestion = () => {
    ...
    if (!editingQuestion.question || !editingQuestion.correctAnswer) {
      toast({
        title: "Validation Error",
        description: "Please fill in all required fields.",
        variant: "destructive",
    ...
    if (editingQuestion.type === "multiple_choice" && editingQuestion.options.some(opt => !opt)) {
      toast({
        title: "Validation Error",
        description: "Please fill in all options for multiple choice questions.",
        variant: "destructive",
