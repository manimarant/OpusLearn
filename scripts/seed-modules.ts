...
async function main() {
  ...
  // Add modules and lessons to each course
  for (const course of courses) {
    console.log(`Adding modules to course: ${course.title}`);
  ...
  console.log('Successfully added modules and lessons to all courses!');
  if (db.pool) await db.pool.end();
}

main().catch((err) => {
  console.error('Error seeding modules and lessons:', err);
  process.exit(1);
}); 